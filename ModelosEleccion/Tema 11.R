...
data <- read_dta("inmates.dta") %>%
  mutate(age2 = age*age) %>%
  dplyr::select(prison, educ, age, age2, AfAm) %>%
  sample_n(., 100000, replace = F)

# Exportando la base de datos para trabajarla en Stata
library("writexl")
write_xlsx(data,"C:/Users/ggarci24/OneDrive - Universidad EAFIT/EAFIT/Cursos EAFIT/Econometria II/R/Tema 11/data.xlsx")
...
# Efecto parcial en el promedio
margins(probit, at = list(educ=mean(data$educ), 
                          age=mean(data$age),
                          AfAm=c("0","1")))
...
ggplot(newdata) +
     ...
     labs(x="Años de educación", y="Prob(Preso=1)") +
