...
# Heterogeneidad a través de aerolíneas (unidades de corte transversal)
df1 <- data %>% group_by(i) %>%
...
# Definimos las variables de corte transversal y tiempo
data2 <- pdata.frame(data, c("i","t"))
...
# El efecto de las variables Xs mediado por las diferencias a trav?s de las unidades
# Adicionando dummy para cada aerol?nea se est? estimando el efecto puro
...
ggplot(data, aes(x = q, y = c, color = factor(i), shape = factor(i))) + 
  geom_point(size=1, shape=21) + 
  theme(legend.position = "none") +
  labs(x="Producci?n", y="Costos totales") +
  #scale_color_manual(values = c("black","blue", "red","yellow","orange","green")) +
  geom_text(aes(label=i), size=3, nudge_y = 200000) +
  geom_smooth(method = "lm", se = T, color='red', linetype="dashed")
...
qf(.05, 5, 81, lower.tail=FALSE) # Valor cr?tico al 5% 
...
# errores Uit est?n correlacionados con los regresores, Ho es que ellos
# no lo est?n
